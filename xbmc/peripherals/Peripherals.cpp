...
void CPeripherals::GetSettingsFromMappingsFile(TiXmlElement *xmlNode, std::map<std::string, PeripheralDeviceSetting> &settings)
{
  ...
  while (currentNode)
  {
    ...
    {
      float fValue = currentNode->Attribute("value") ? static_cast<float>(atof(currentNode->Attribute("value"))) : 0;
      float fMin   = currentNode->Attribute("min") ? static_cast<float>(atof(currentNode->Attribute("min"))) : 0;
      float fStep  = currentNode->Attribute("step") ? static_cast<float>(atof(currentNode->Attribute("step"))) : 0;
      float fMax   = currentNode->Attribute("max") ? static_cast<float>(atof(currentNode->Attribute("max"))) : 0;
