...
NPT_Result CUPnPServer::SetupServices()
{
  ...
  if (service)
  {
    ...
    service->SetStateVariable(
        "SortCapabilities",
        "res@duration,res@size,res@bitrate,dc:date,dc:title,dc:size,upnp:album,upnp:artist,upnp:"
        "albumArtist,upnp:episodeNumber,upnp:genre,upnp:originalTrackNumber,upnp:rating,upnp:"
        "episodeCount,upnp:episodeSeason,xbmc:rating,xbmc:dateadded,xbmc:votes");
  }
...
void CUPnPServer::PropagateUpdates()
{
  ...
  if (m_scanning || !CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(
                        CSettings::SETTING_SERVICES_UPNPANNOUNCE))
    return;
  ...
  NPT_CHECK_LABEL(service->SetStateVariable("ContainerUpdateIDs",
                                            buffer.substr(0, buffer.size() - 1).c_str(), true),
                  failed);
                                         ...
                                         const PLT_HttpRequestContext& context)
{
  ...
  m_logger->info(
      "Received UPnP Browse Metadata request for encoded object '{}' (plain value: '{}')",
      object_id, DecodeObjectId(object_id).GetChars());
                                               ...
                                               const PLT_HttpRequestContext& context)
{
  ...
  m_logger->info("Received Browse DirectChildren request for encoded object '{}' (plain value: "
                 "'{}'), with sort criteria {}",
                                      ...
                                      const char* parent_id /* = NULL */)
{
  ...
  NPT_UInt32 max_count = (requested_count == 0) ? m_MaxReturnedItems
                                                : std::min((unsigned long)requested_count,
                                                           (unsigned long)m_MaxReturnedItems);
  NPT_UInt32 stop_index = std::min((unsigned long)(starting_index + max_count),
                                   (unsigned long)items.Size()); // don't return more than we can
                                          ...
                                          const PLT_HttpRequestContext& context)
{
  ...
  else if (searchClass.Find("object.item.audioItem") >= 0)
  {
    ...
    artist =
        artist.GetLength() ? artist : FindSubCriteria(searchClass, "microsoft:artistPerformer");
    artist =
        artist.GetLength() ? artist : FindSubCriteria(searchClass, "microsoft:artistAlbumArtist");
                                       ...
                                       const PLT_HttpRequestContext& context)
{
  ...
  if (IsVideoDb(updated))
  {
    ...
    if (!playCount.IsEmpty() && playCount.Compare(current_vals["playCount"]) != 0)
    {
      ...
      if (!lastPlayed.IsEmpty() && lastPlayed.Compare(current_vals["lastPlaybackTime"]) != 0)
                                  ...
                                  const NPT_String& md5)
{
  ...
  // File requested
  NPT_HttpUrl rooturi(context.GetLocalAddress().GetIpAddress().ToString(),
  ...
  if (file_path.Left(8).Compare("stack://", true) == 0)
  {
    ...
    for (; url; url++)
    {
      ...
      output += "\r\n";
      output +=
          BuildSafeResourceUri(rooturi, context.GetLocalAddress().GetIpAddress().ToString(), *url);
      output += "\r\n";
    }
  ...
  return PLT_HttpServer::ServeFile(request, context, response, file_path);
}
