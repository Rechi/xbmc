...
bool CNetworkInterfaceAndroid::GetHostMacAddress(unsigned long host_ip, std::string& mac) const
{
  ...
  mac = StringUtils::Format("%02X:%02X:%02X:%02X:%02X:%02X",
    static_cast<uint8_t>(res->sa_data[0]), static_cast<uint8_t>(res->sa_data[1]), static_cast<uint8_t>(res->sa_data[2]),
    static_cast<uint8_t>(res->sa_data[3]), static_cast<uint8_t>(res->sa_data[4]), static_cast<uint8_t>(res->sa_data[5]));
