  ...
  if( m_glView && [m_glView isXBMCAlive] )//NO GESTURES BEFORE WE ARE UP AND RUNNING
  {
    ...
    if( [sender state] == UIGestureRecognizerStateChanged && sender.numberOfTouches )
    {
      ...
      if( bNotify )
      {
        ...
        CGenericTouchActionHandler::GetInstance().OnTouchGesturePan(static_cast<float>(point.x), static_cast<float>(point.y),
                                                            static_cast<float>(xMovement), static_cast<float>(yMovement),
    ...
    if( touchBeginSignaled && ([sender state] == UIGestureRecognizerStateEnded || [sender state] == UIGestureRecognizerStateCancelled))
    {
      ...
      CGenericTouchActionHandler::GetInstance().OnTouchGestureEnd(static_cast<float>(lastGesturePoint.x), static_cast<float>(lastGesturePoint.y),
...
- (IBAction)handleTap:(UIGestureRecognizer *)sender
{
  ...
  if( ([m_glView isReadyToRun] && [sender numberOfTouches] == 1) || [m_glView isXBMCAlive])
  {
    ...
    CGenericTouchActionHandler::GetInstance().OnTap(static_cast<float>(point.x), static_cast<float>(point.y), [sender numberOfTouches]);
  }
}
//--------------------------------------------------------------
- (IBAction)handleSingleFingerSingleLongTap:(UIGestureRecognizer *)sender
{
  if( m_glView && [m_glView isXBMCAlive] && sender.numberOfTouches)//NO GESTURES BEFORE WE ARE UP AND RUNNING
  {
    ...
    {
      CGenericTouchActionHandler::GetInstance().OnSingleTouchMove(static_cast<float>(point.x), static_cast<float>(point.y), point.x - lastGesturePoint.x, point.y - lastGesturePoint.y, 0, 0);
    }

    if (sender.state == UIGestureRecognizerStateEnded)
    {
      CGenericTouchActionHandler::GetInstance().OnLongPress(static_cast<float>(point.x), static_cast<float>(point.y));
    }
  }
}
//--------------------------------------------------------------
