      ...
      if (disk)
      {
        ...
        if (details)
        {
          if (kCFBooleanTrue == CFDictionaryGetValue(details, kDADiskDescriptionMediaRemovableKey))
          {
            ...
            {
              CFStringRef mediaKind = static_cast<CFStringRef>(CFDictionaryGetValue(details, kDADiskDescriptionMediaKindKey));
...
namespace
{
    ...
    m_runloopSource(CFRunLoopSourceCreate(kCFAllocatorDefault, 0, &m_runLoopSourceContext))
  {
    ...
    {
      CFURLRef url = CFURLCreateFromFileSystemRepresentation(kCFAllocatorDefault, reinterpret_cast<const UInt8 *>(mountpath.c_str()), mountpath.size(), TRUE);
      if (url)
      {
