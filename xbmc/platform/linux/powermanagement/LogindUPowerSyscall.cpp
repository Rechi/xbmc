...
void CLogindUPowerSyscall::UpdateBatteryLevel()
{
  ...
  if (batteryCount > 0)
  {
    m_batteryLevel = static_cast<int>(batteryLevelSum / static_cast<double>(batteryCount));
  ...
  if (m_connection)
  {
    ...
    if (msg)
    {
      if (dbus_message_is_signal(msg.get(), "org.freedesktop.login1.Manager", "PrepareForSleep"))
      {
        ...
        CLog::Log(LOGDEBUG, "LogindUPowerSyscall: Received PrepareForSleep with arg %i", static_cast<int>(arg));
        if (arg)
        {
          callback->OnSleep();
