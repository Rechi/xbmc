...
const std::string CPictureInfoTag::GetInfo(int info) const
{
  ...
  case SLIDESHOW_EXIF_COMMENT:
    ...
    {
      g_charsetConverter.ucs2ToUTF8(
          std::u16string(reinterpret_cast<const char16_t*>(m_exifInfo.Comments)), value);
    }
    else
    {
  ...
  case SLIDESHOW_EXIF_XPCOMMENT:
    ...
    {
      g_charsetConverter.ucs2ToUTF8(
          std::u16string(reinterpret_cast<const char16_t*>(m_exifInfo.XPComment)), value);
    }
    else
    {
  ...
  case SLIDESHOW_EXIF_EXPOSURE_TIME:
    if (m_exifInfo.ExposureTime)
    {
      ...
      if (m_exifInfo.ExposureTime <= 0.5)
        value +=
            StringUtils::Format(" (1/%d)", static_cast<int>(0.5 + 1 / m_exifInfo.ExposureTime));
    }
    break;
