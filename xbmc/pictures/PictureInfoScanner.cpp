...
void CPictureInfoScanner::Process()
{
  ...
  {
    if (m_showDialog &&
        !CServiceBroker::GetSettings().GetBool(CSettings::SETTING_PICTURELIBRARY_BACKGROUNDUPDATE))
    {
    ...
    CLog::Log(LOGNOTICE, "PictureInfoScanner: Starting scan ..");
    ANNOUNCEMENT::CAnnouncementManager::GetInstance().Announce(ANNOUNCEMENT::PictureLibrary, "xbmc",
                                                               "OnScanStarted");
    ...
    CLog::Log(LOGNOTICE, "PicktureInfoScanner: Finished scan. Scanning for pickture info took %s",
...
void CPictureInfoScanner::Start(const std::string& strDirectory, bool scanAll)
{
  ...
  m_database.Close();
  m_bClean = g_advancedSettings.m_bPictureLibraryCleanOnUpdate;
