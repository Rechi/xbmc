  ...
  if (window)
  {
    ...
    else
    {
      ...
      m_info.currentWind = StringUtils::Format(g_localizeStrings.Get(434).c_str(),
                                               direction.c_str(), static_cast<int>(speed.To(g_langInfo.GetSpeedUnit())), g_langInfo.GetSpeedUnitString().c_str());
    }
    std::string windspeed = StringUtils::Format("%i %s", static_cast<int>(speed.To(g_langInfo.GetSpeedUnit())), g_langInfo.GetSpeedUnitString().c_str());
