...
void CViewDatabase::UpdateTables(int version)
{
  ...
  if (version < 6)
  {
    ...
    while (!m_pDS->eof())
    {
      SortDescription sorting = SortUtils::TranslateOldSortMethod(static_cast<SORT_METHOD>(m_pDS->fv(4).get_asInt()));
      ...
      std::string sql = PrepareSQL("INSERT INTO view (idView, window, path, viewMode, sortMethod, sortOrder, sortAttributes, skin) VALUES (%i, %i, '%s', %i, %i, %i, %i, '%s')",
        ...
        static_cast<int>(sorting.sortBy), m_pDS->fv(5).get_asInt(), static_cast<int>(sorting.sortAttributes), m_pDS->fv(6).get_asString().c_str());
      m_pDS2->exec(sql);
...
bool CViewDatabase::GetViewState(const std::string &path, int window, CViewState &state, const std::string &skin)
{
  try
  {
    ...
    { // have some information
      ...
      state.m_sortDescription.sortOrder = static_cast<SortOrder>(m_pDS->fv("sortOrder").get_asInt());
      state.m_sortDescription.sortAttributes = static_cast<SortAttribute>(m_pDS->fv("sortAttributes").get_asInt());
      m_pDS->close();
      return true;
    }
    m_pDS->close();
  ...
  try
  {
    ...
    { // update the view
      ...
      sql = PrepareSQL("update view set viewMode=%i,sortMethod=%i,sortOrder=%i,sortAttributes=%i where idView=%i",
        state.m_viewMode, static_cast<int>(state.m_sortDescription.sortBy), static_cast<int>(state.m_sortDescription.sortOrder), static_cast<int>(state.m_sortDescription.sortAttributes), idView);
      m_pDS->exec(sql);
    }
    else
    { // add the view
      ...
      sql = PrepareSQL("insert into view (idView, path, window, viewMode, sortMethod, sortOrder, sortAttributes, skin) values(NULL, '%s', %i, %i, %i, %i, %i, '%s')",
        path1.c_str(), window, state.m_viewMode, static_cast<int>(state.m_sortDescription.sortBy), static_cast<int>(state.m_sortDescription.sortOrder), static_cast<int>(state.m_sortDescription.sortAttributes), skin.c_str());
      m_pDS->exec(sql);
    }
  }
  catch (...)
