...
namespace XbmcCommons
{
  ...
  public:
     ...
     */
    inline Buffer(void* buffer_, size_t bufferSize) : buffer(static_cast<unsigned char*>(buffer_)), mcapacity(bufferSize)
    {
      clear();
    }
...
#undef DEFAULTBUFFERRELATIVERW
    ...
    inline std::string getString() { std::string ret(reinterpret_cast<const char*>(buffer + mposition)); size_t len = ret.length() + 1; check(len); mposition += len; return ret; }
    ...
    inline char* getCharPointerDirect() { char* ret = reinterpret_cast<char*>(buffer + mposition); size_t len = strlen(ret) + 1; check(len); mposition += len; return ret; }
