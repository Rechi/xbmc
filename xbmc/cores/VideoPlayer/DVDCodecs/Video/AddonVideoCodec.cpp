...
CDVDVideoCodec::VCReturn CAddonVideoCodec::GetPicture(VideoPicture* pVideoPicture)
{
  ...
  case VIDEOCODEC_RETVAL::VC_PICTURE:
    ...
    {
      pVideoPicture->iDisplayWidth = (static_cast<int>(lrint(pVideoPicture->iHeight * m_displayAspect))) & ~3;
      if (pVideoPicture->iDisplayWidth > pVideoPicture->iWidth)
      {
        pVideoPicture->iDisplayWidth = pVideoPicture->iWidth;
        pVideoPicture->iDisplayHeight = (static_cast<int>(lrint(pVideoPicture->iWidth / m_displayAspect))) & ~3;
      }
    }
