...
{
  m_StreamId = streamId;
  CLog::Log(LOGDEBUG, "CCoreAudioStream::Open: Opened stream 0x%04x.",
            static_cast<uint>(m_StreamId));
...
void CCoreAudioStream::Close(bool restore)
{
  ...
  if (restore && m_OriginalVirtualFormat.mFormatID && m_StreamId)
  {
    CLog::Log(LOGDEBUG,
              ...
              static_cast<uint>(m_StreamId),
              StreamDescriptionToString(m_OriginalVirtualFormat, formatString));
  ...
  if (restore && m_OriginalPhysicalFormat.mFormatID && m_StreamId)
  {
    CLog::Log(LOGDEBUG,
              ...
              static_cast<uint>(m_StreamId),
              StreamDescriptionToString(m_OriginalPhysicalFormat, formatString));
  ...
  m_OriginalVirtualFormat.mFormatID  = 0;
  m_OriginalPhysicalFormat.mFormatID = 0;
  CLog::Log(LOGDEBUG, "CCoreAudioStream::Close: Closed stream 0x%04x.",
            static_cast<uint>(m_StreamId));
  m_StreamId = 0;
}
  ...
  if (!m_OriginalVirtualFormat.mFormatID)
  {
    ...
    if (!GetVirtualFormat(&m_OriginalVirtualFormat))
    {
      CLog::Log(LOGERROR,
                ...
                "Unable to retrieve current virtual format for stream 0x%04x.",
                static_cast<uint>(m_StreamId));
      return false;
    }
  }
  ...
  for (int i = 0; i < 10; ++i)
  {
        ...
        checkVirtualFormat.mFramesPerPacket == pDesc->mFramesPerPacket)
    {
      ...
      CLog::Log(LOGDEBUG,
                ...
                static_cast<uint>(m_StreamId),
                StreamDescriptionToString(checkVirtualFormat, formatString));
      break;
    }
  ...
  for(int i = 0; i < 10; ++i)
  {
        ...
        checkPhysicalFormat.mChannelsPerFrame == pDesc->mChannelsPerFrame)
    {
      ...
      CLog::Log(LOGDEBUG,
                ...
                static_cast<uint>(m_StreamId),
                StreamDescriptionToString(checkPhysicalFormat, formatString));
      break;
    }
