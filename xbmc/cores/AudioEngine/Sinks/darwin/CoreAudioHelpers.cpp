...
std::string GetError(OSStatus error)
{
  ...
  *reinterpret_cast<UInt32*>(buffer + 1) = CFSwapInt32HostToBig(error);
  ...
  else
  {
    ...
    snprintf(buffer, sizeof(buffer), "%d", static_cast<(int>(error));
  }
...
{
  char fourCC[5] = {static_cast<char>((desc.mFormatID >> 24) & 0xFF),
                    static_cast<char>((desc.mFormatID >> 16) & 0xFF),
                    static_cast<char>((desc.mFormatID >> 8) & 0xFF),
                    static_cast<char>(desc.mFormatID & 0xFF), 0};
  ...
  switch (desc.mFormatID)
  {
    case kAudioFormatLinearPCM:
      sstr << "[" << fourCC << "] "
           ...
           << "Interleaved " << desc.mChannelsPerFrame << " Channel " << desc.mBitsPerChannel
           << "-bit "
           << ((desc.mFormatFlags & kAudioFormatFlagIsFloat) ? "Floating Point "
                                                             : "Signed Integer ")
           << ((desc.mFormatFlags & kAudioFormatFlagIsBigEndian) ? "BE" : "LE") << " ("
           << static_cast<UInt32>(desc.mSampleRate) << "Hz)";
      str = sstr.str();
      break;
    case kAudioFormatAC3:
      sstr << "[" << fourCC << "] "
           << ((desc.mFormatFlags & kAudioFormatFlagIsBigEndian) ? "BE" : "LE") << " AC-3/DTS ("
           << static_cast<UInt32>(desc.mSampleRate) << "Hz)";
      str = sstr.str();
      break;
