...
inline void SwapEndian(uint16_t* dst, const uint16_t* src, unsigned int size)
{
...
#else
  size += size & 0x1;
  SwapEndian(reinterpret_cast<uint16_t*>(packet->m_data), reinterpret_cast<const uint16_t*>(data),
             size >> 1);
#endif
