...
uintptr_t create_dummy_function(const char* strDllName, const char* strFunctionName)
{
  ...
  char* pData = static_cast<char*>(malloc(iFunctionSize + 12 + iDllNameSize + iFunctionNameSize));
  if (!pData)
    return 0;
  ...
  *reinterpret_cast<int*>(pData + 4) = (intptr_t)offDataPointer1;
  *reinterpret_cast<int*>(pData + 10) = (intptr_t)offDataPointer2;
  *reinterpret_cast<int*>(pData + 17) = (intptr_t)offDataPointer3;

  // 2 fill datapointer with pointer to 5 (string)
  *reinterpret_cast<int*>(offDataPointer1) = (intptr_t)offStringFunc;
  // 3 fill datapointer with pointer to 6 (string)
  *reinterpret_cast<int*>(offDataPointer2) = (intptr_t)offStringDll;
  // 4 fill datapointer with pointer to dll_dummy_output
  *reinterpret_cast<int*>(offDataPointer3) = (intptr_t)dll_dummy_output;
