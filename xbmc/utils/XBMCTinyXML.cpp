...
bool CXBMCTinyXML::LoadFile(const std::string& _filename, TiXmlEncoding encoding)
{
  ...
  {
    SetError(TIXML_ERROR_OPENING_FILE, NULL, nullptr, TIXML_ENCODING_UNKNOWN);
    return false;
  }
...
bool CXBMCTinyXML::InternalParse(const std::string& rawdata, TiXmlEncoding encoding /*= TIXML_DEFAULT_ENCODING */)
{
  ...
  if (pos == std::string::npos)
    return (TiXmlDocument::Parse(rawdata.c_str(), nullptr, encoding) !=
            NULL); // nothing to fix, process data directly
  ...
  return (TiXmlDocument::Parse(data.c_str(), nullptr, encoding) != NULL);
}
