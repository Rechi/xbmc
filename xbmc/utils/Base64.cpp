...
void Base64::Decode(const char* input, unsigned int length, std::string &output)
{
  ...
  for (unsigned int i = 0; i < length; i += 4)
  {
    ...
    l |= (((i + 1) < length) ? (((static_cast<unsigned long>(m_characters.find(input[i + 1]))) & 0x3F) << 12) : 0);
    l |= (((i + 2) < length) ? (((static_cast<unsigned long>(m_characters.find(input[i + 2]))) & 0x3F) <<  6) : 0);
    l |= (((i + 3) < length) ? (((static_cast<unsigned long>(m_characters.find(input[i + 3]))) & 0x3F) <<  0) : 0);
