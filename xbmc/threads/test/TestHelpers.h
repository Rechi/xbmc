...
inline static bool waitForThread(volatile std::atomic<long>& mutex, int numWaiters, int milliseconds)
{
...
class AtomicGuard
{
  volatile std::atomic<long>* val;
public:
  inline AtomicGuard(volatile std::atomic<long>* val_) : val(val_) { if (val) ++(*val); }
