...
void CGUIDialogTeletext::SetCoordinates()
{
  ...
  right = CServiceBroker::GetWinSystem()->GetGfxContext().ScaleFinalXCoord(static_cast<float>(m_coordsRes.iWidth), 0);
  top = CServiceBroker::GetWinSystem()->GetGfxContext().ScaleFinalYCoord(0, 0);
  bottom = CServiceBroker::GetWinSystem()->GetGfxContext().ScaleFinalYCoord(0, static_cast<float>(m_coordsRes.iHeight));
