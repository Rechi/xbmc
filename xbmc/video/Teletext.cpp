  ...
  if (m_txtCache->PageUpdate && m_txtCache->PageReceiving != m_txtCache->Page && m_RenderInfo.InputCounter == 2)
  {
    ...
    if (m_txtCache->SubPageTable[m_txtCache->Page] != 0xFF)
    {
      ...
      if (m_RenderInfo.Boxed || m_RenderInfo.TranspMode)
        ...
        FillBorder(GetColorRGB(static_cast<enumTeletextColor>(m_txtCache->FullScrColor)));
...
void CTeletextDecoder::Showlink(int column, int linkpage)
{
  ...
  else /* display number */
  {
    ...
    {
      line[6] = '<';
      CDVDTeletextTools::Hex2Str(reinterpret_cast<char*>(line) + 5, linkpage);
    }
    else
      CDVDTeletextTools::Hex2Str(reinterpret_cast<char*>(line) + 6, linkpage);
...
void CTeletextDecoder::CopyBB2FB()
{
  ...
  if (m_RenderInfo.TranspMode)
    ...
    fillcolor = GetColorRGB(static_cast<enumTeletextColor>(m_txtCache->FullScrColor));
...
{
  FT_Error result = FT_New_Face(library, static_cast<const char*>(face_id), 0, aface);

  if (!result)
    CLog::Log(LOGINFO, "Teletext font %s loaded", static_cast<char*>(face_id));
  else
    CLog::Log(LOGERROR, "Opening of Teletext font %s failed", static_cast<char*>(face_id));
...
void CTeletextDecoder::RenderCharIntern(TextRenderInfo_t* RenderInfo, int Char, TextPageAttr_t *Attribute, int zoom, int yoffset)
{
  ...
  fgcolor = GetColorRGB(static_cast<enumTeletextColor>(Attribute->fg));
  ...
  {
    bgcolor = GetColorRGB(static_cast<enumTeletextColor>(Attribute->bg));
  }
        ...
        else
        {
          ...
          else
          {
            ...
            if (h != 0xFF)  /* first parity error: try dehamming */
              CDVDTeletextTools::Hex2Str(reinterpret_cast<char*>(p), h);
            else
              *p = ' ';
          }
        }
