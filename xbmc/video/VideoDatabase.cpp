...
void CVideoDatabase::CreateTables()
{
  ...
  m_pDS->exec("CREATE TABLE bookmark ( idBookmark integer primary key, idFile integer, idUser integer, timeInSeconds double, totalTimeInSeconds double, thumbNailImage text, player text, playerState text, type integer)\n");
...
bool CVideoDatabase::GetFileInfo(const std::string& strFilenameAndPath, CVideoInfoTag& details, int idFile /* = -1 */)
{
  try
  {
    ...
    std::string sql = PrepareSQL("SELECT * FROM files "
                                ...
                                "WHERE files.idFile = %i", CBookmark::RESUME, idFile, CProfilesManager::GetInstance().GetCurrentProfileId());
    if (!m_pDS->query(sql))
      return false;
...
void CVideoDatabase::GetBookMarksForFile(const std::string& strFilenameAndPath, VECBOOKMARKS& bookmarks, CBookmark::EType type /*= CBookmark::STANDARD*/, bool bAppend, long partNumber)
{
  try
  {
    ...
    else
    {
      ...
      std::string strSQL=PrepareSQL("select * from bookmark where idFile=%i and type=%i and idUser=%i order by timeInSeconds", idFile, (int)type, CProfilesManager::GetInstance().GetCurrentProfileId());
      m_pDS->query( strSQL );
      while (!m_pDS->eof())
      {
        CBookmark bookmark;
...
void CVideoDatabase::DeleteResumeBookMark(const CFileItem& item)
{
  ...
  try
  {
    std::string sql = PrepareSQL("delete from bookmark where idFile=%i and type=%i and idUser=%i", fileID, CBookmark::RESUME, CProfilesManager::GetInstance().GetCurrentProfileId());
    m_pDS->exec(sql);
...
void CVideoDatabase::AddBookMarkToFile(const std::string& strFilenameAndPath, const CBookmark &bookmark, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  try
  {
    ...
    {
      strSQL=PrepareSQL("select idBookmark from bookmark where idFile=%i and type=1 and idUser=%i", idFile, CProfilesManager::GetInstance().GetCurrentProfileId());
    }
    else if (type == CBookmark::STANDARD) // get the same bookmark again, and update. not sure here as a dvd can have same time in multiple places, state will differ thou
    {
      ...
      strSQL=PrepareSQL("select idBookmark from bookmark where idFile=%i and type=%i and (timeInSeconds between %f and %f) and playerState='%s' and idUser=%i", idFile, (int)type, mintime, maxtime, bookmark.playerState.c_str(), CProfilesManager::GetInstance().GetCurrentProfileId());
    }
    ...
    if (idBookmark >= 0 )
      ...
      strSQL=PrepareSQL("insert into bookmark (idBookmark, idFile, idUser, timeInSeconds, totalTimeInSeconds, thumbNailImage, player, playerState, type) values(NULL,%i,%i,%f,%f,'%s','%s','%s', %i)", idFile, CProfilesManager::GetInstance().GetCurrentProfileId(), bookmark.timeInSeconds, bookmark.totalTimeInSeconds, bookmark.thumbNailImage.c_str(), bookmark.player.c_str(), bookmark.playerState.c_str(), (int)type);
...
void CVideoDatabase::ClearBookMarkOfFile(const std::string& strFilenameAndPath, CBookmark& bookmark, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  try
  {
    ...
    double mintime = bookmark.timeInSeconds - 0.5f;
    double maxtime = bookmark.timeInSeconds + 0.5f;
    std::string strSQL = PrepareSQL("select idBookmark from bookmark where idFile=%i and type=%i and playerState like '%s' and player like '%s' and (timeInSeconds between %f and %f) and idUser=%i", idFile, type, bookmark.playerState.c_str(), bookmark.player.c_str(), mintime, maxtime, CProfilesManager::GetInstance().GetCurrentProfileId());
...
void CVideoDatabase::ClearBookMarksOfFile(int idFile, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  ...
  try
  {
    ...
    std::string strSQL=PrepareSQL("delete from bookmark where idFile=%i and type=%i and idUser=%i", idFile, (int)type, CProfilesManager::GetInstance().GetCurrentProfileId());
    m_pDS->exec(strSQL);
    if (type == CBookmark::EPISODE)
    {
...
bool CVideoDatabase::GetResumePoint(CVideoInfoTag& tag)
{
  ...
  try
  {
    ...
    {
      std::string strSQL=PrepareSQL("select timeInSeconds, totalTimeInSeconds from bookmark where idFile=%i and type=%i and idUser=%i order by timeInSeconds", tag.m_iFileId, CBookmark::RESUME, CProfilesManager::GetInstance().GetCurrentProfileId());
      m_pDS2->query( strSQL );
      if (!m_pDS2->eof())
      {
...
bool CVideoDatabase::GetPlayCounts(const std::string &strPath, CFileItemList &items)
{
  ...
  try
  {
    ...
    if (URIUtils::IsPlugin(strPath))
    {
      for (auto& item : items)
      {
        ...
        m_pDS->query(PrepareSQL(sql +
          ...
          (int)CBookmark::RESUME, CProfilesManager::GetInstance().GetCurrentProfileId(), filename.c_str(), path.c_str()));
    ...
    else
    {
      ...
      sql = PrepareSQL(sql + "WHERE files.idPath=%i", (int)CBookmark::RESUME, CProfilesManager::GetInstance().GetCurrentProfileId(), pathID);
